...
def fetch_krx_data(code: str, s_str: str, e_str: str, interval: str, extra_data: list | tuple, include_nxt: bool = False) -> tuple[pd.DataFrame, str]:
    ...
    try:
        from kis_api import fetch_intraday_history, fetch_daily_history
        import FinanceDataReader as fdr

        if interval in INTRADAY_INTERVALS:
            # Get 1m data directly from KIS (this bypasses yfinance's 20min delay entirely)
            ...
            # fetch_daily_history automatically handles the <100 length limitation of KIS API natively
            start_str = start_d.strftime("%Y%m%d")
            # If Yearly or comprehensive, override start date to 1990
