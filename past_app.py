...
def append_live_minute_data(df, ticker, m_name=None):
    ...
    try:
        ...
        if not live_df.empty:
            ...
            new_idx = pd.to_datetime(last_row.index[0]).tz_localize(None)
...
def render_krx_inputs_fragment(sorted_names, name_to_ticker, default_index):
    ...
    with col2:
        ...
        # Holding Status
        # st.pills("투자 자산 보유 (Holding Status)", ["보유(매도예정)", "미보유(매수예정)", "관망(중립)"], selection_mode="single", default="관망(중립)", key="kr_holding_status")
...
def render_us_inputs_fragment(us_sorted_names, us_name_to_ticker, default_idx):
    ...
    with col2:
        ...
        # Holding Status
        # st.pills("투자 자산 보유 (Holding Status)", ["보유(매도예정)", "미보유(매수예정)", "관망(중립)"], selection_mode="single", default="관망(중립)", key="us_holding_status")
...
def run_analysis_and_prompts(df, ticker, name, market, currency, interval_label, ranks=None, key_suffix="", selected_data=None, holding_status="관망(중립)"):
    ...
    @st.fragment
    def render_holding_ui(ticker, interval_label, key_suffix, initial_holding_status, currency):
        ...
        holding_status = st.pills(
            ...
            default=st.session_state.get(h_key, initial_holding_status),
            key=h_key
        )
        ...
        # We store these explicitly to be used sequentially
        st.session_state[f"val_{h_key}"] = holding_status
        st.session_state[f"val_avg_price_{ticker}_{interval_label}_{key_suffix}"] = avg_price
        
    # 2. AI Prompts & Analysis
    def render_ai_analysis_content(ticker, name, market, currency, interval_label, display_df, key_suffix, initial_holding_status):
        
        render_holding_ui(ticker, interval_label, key_suffix, initial_holding_status, currency)
        
        holding_status = st.session_state.get(f"val_holding_{ticker}_{interval_label}_{key_suffix}", "관망(중립)")
        avg_price = st.session_state.get(f"val_avg_price_{ticker}_{interval_label}_{key_suffix}", None)
    ...
    render_ai_analysis_content(ticker, name, market, currency, interval_label, display_df, key_suffix, holding_status)
  ...
  <div style="flex: 0 0 165px; display: flex; flex-direction: column; justify-content: center; font-size: 13px; margin-top: 5px; min-width: 0;">
    ...
    if st.session_state.get('run_krx'):
        ...
        with st.spinner('KRX 데이터 가져오는 중...'):
            ...
            holding_status = st.session_state.get("kr_holding_status", "관망(중립)")
            ...
            def fetch_krx_data(code, start_d, end_d, interval, extra_data):
                df = pd.DataFrame()
                m_name = "KRX"
                
                # Convert dates to string for PyKRX
            ...
            df_kr, market_name = fetch_krx_data(kr_code, start_date_kr, end_date_kr, interval_kr_sel, extra_data_sel)
            ...
            try:
                if not df_kr.empty:
                    ...
                    if interval_kr_sel == "일/주/월/연봉 종합분석":
                        ...
                        with t1:
                            @st.fragment
                            def render_multi_holding_ui(code, currency, initial_holding_status):
                                h_key = f"kr_multi_{code}_holding"
                                holding_status = st.pills(
                                    "💡 투자 자산 보유 상태 (Holding Status)", 
                                    ["보유(매도예정)", "미보유(매수예정)", "관망(중립)"], 
                                    selection_mode="single", 
                                ...
                                if holding_status == "보유(매도예정)":
                                    ...
                                    avg_price = st.number_input(
                                        f"현재 평단가 입력 ({currency}) - 선택사항", 
                                        min_value=0.0, 
                                        value=st.session_state.get(f"avg_price_kr_multi_{code}", 0.0), 
                                        step=step_val,
                                        key=f"avg_price_kr_multi_{code}",
                                        help="평단가를 입력하시면 AI가 평단가 대비 수익 실현/손실 최소화 전략을 상세히 분석합니다. (0 입력 시 미반영)"
                                    )
                                
                                st.session_state[f"val_{h_key}"] = holding_status
                                st.session_state[f"val_avg_price_kr_multi_{code}"] = avg_price

                            def render_multi_ai_content(code, name, market, currency, dfs, news, initial_holding_status):
                                render_multi_holding_ui(code, currency, initial_holding_status)
                                
                                holding_status = st.session_state.get(f"val_kr_multi_{code}_holding", "관망(중립)")
                                avg_price = st.session_state.get(f"val_avg_price_kr_multi_{code}", None)
                            ...
                            news_list = []
                            render_multi_ai_content(kr_code, selected_name, market_name, "KRW", timeframe_dfs, news_list, holding_status)

                        with t2:
                             st.dataframe(df_d)
                             run_analysis_and_prompts(df_d, kr_code, selected_name, market_name, "KRW", "일봉", ranks, key_suffix="kr_d", selected_data=extra_data_sel, holding_status=holding_status)
                        with t3: 
                            st.dataframe(df_w)
                            run_analysis_and_prompts(df_w, kr_code, selected_name, market_name, "KRW", "주봉", ranks, key_suffix="kr_w", selected_data=extra_data_sel, holding_status=holding_status)
                        with t4: 
                            st.dataframe(df_m)
                            run_analysis_and_prompts(df_m, kr_code, selected_name, market_name, "KRW", "월봉", ranks, key_suffix="kr_m", selected_data=extra_data_sel, holding_status=holding_status)
                        with t5: 
                            st.dataframe(df_y)
                            run_analysis_and_prompts(df_y, kr_code, selected_name, market_name, "KRW", "연봉", ranks, key_suffix="kr_y", selected_data=extra_data_sel, holding_status=holding_status)
                        ...
                        run_analysis_and_prompts(df_final, kr_code, selected_name, market_name, "KRW", interval_kr_sel, ranks, key_suffix="kr_single", selected_data=extra_data_sel, holding_status=holding_status)
...
with tab_us:
    ...
    if st.session_state.get('run_us'):
        if us_name_to_ticker:
            ...
            us_ticker = st.session_state.get("us_ticker_input", "AAPL")
            
        # Retrieve Holding Status
        holding_status = st.session_state.get("us_holding_status", "관망(중립)")
            
        with st.spinner('미국 주식 데이터 가져오는 중...'):
            start_load_time = time.time()
            # Use Local Dates
            @st.cache_data(ttl=300, show_spinner=False)
            def fetch_us_data(ticker, start_d, end_d, interval):
                df = pd.DataFrame()
                fetch_int = "1d"
            ...
            df_us = fetch_us_data(us_ticker, start_date_us, end_date_us, interval_us_sel)
            ...
            if not df_us.empty:
                if interval_us_sel == "일/주/월/연봉 종합분석":
                    ...
                    with t1:
                        @st.fragment
                        def render_us_multi_holding_ui(code, currency, initial_holding_status):
                            h_key = f"us_multi_{code}_holding"
                            holding_status = st.pills(
                                "💡 투자 자산 보유 상태 (Holding Status)", 
                                ["보유(매도예정)", "미보유(매수예정)", "관망(중립)"], 
                                selection_mode="single", 
                            ...
                            if holding_status == "보유(매도예정)":
                                ...
                                avg_price = st.number_input(
                                    f"현재 평단가 입력 ({currency}) - 선택사항", 
                                    min_value=0.0, 
                                    value=st.session_state.get(f"avg_price_us_multi_{code}", 0.0), 
                                    step=step_val,
                                    key=f"avg_price_us_multi_{code}",
                                    help="평단가를 입력하시면 AI가 평단가 대비 수익 실현/손실 최소화 전략을 상세히 분석합니다. (0 입력 시 미반영)"
                                )
                                
                            st.session_state[f"val_{h_key}"] = holding_status
                            st.session_state[f"val_avg_price_us_multi_{code}"] = avg_price

                        def render_multi_ai_content(code, name, market, currency, dfs, news, initial_holding_status):
                            render_us_multi_holding_ui(code, currency, initial_holding_status)
                            
                            holding_status = st.session_state.get(f"val_us_multi_{code}_holding", "관망(중립)")
                            avg_price = st.session_state.get(f"val_avg_price_us_multi_{code}", None)
                            ...
                            if not df_daily.empty:
                                start_dt = df_daily.index.min()
                                end_dt = df_daily.index.max()
                                start_dt_str = end_dt.strftime('%Y-%m-%d %H:%M')
                                end_dt_str = end_dt.strftime('%Y-%m-%d %H:%M')
                            ...
                            if gemini_api_key:
                                with st.spinner("멀티 타임프레임 분석 중..."):
                                    prompt = generate_multi_timeframe_gemini_prompt(code, name, market, currency, dfs, news_list=None, holding_status=holding_status, avg_price=avg_price, start_dt_str=start_dt_str, end_dt_str=end_dt_str)
                                    response = get_gemini_response(prompt, gemini_api_key)
                                    st.markdown(response)
                            else:
                                st.error("API 키가 설정되지 않았습니다.")
                            ...
                            with c1:
                                st.markdown("### 🟢 ChatGPT Plus 용")
                                gpt_us_multi_p = generate_multi_timeframe_chatgpt_prompt(code, name, market, currency, dfs, news_list=None, holding_status=holding_status, avg_price=avg_price, start_dt_str=start_dt_str, end_dt_str=end_dt_str)
                                st.code(gpt_us_multi_p, language=None)
                            with c2:
                                st.markdown("### 🔵 Gemini (Google One) 용")
                                gem_us_multi_p = generate_multi_timeframe_gemini_prompt(code, name, market, currency, dfs, news_list=None, holding_status=holding_status, avg_price=avg_price, start_dt_str=start_dt_str, end_dt_str=end_dt_str)
                                st.code(gem_us_multi_p, language=None)
                            news_list = []
                            render_multi_ai_content(us_ticker, name_diplay, "US", "USD", timeframe_dfs, news_list, holding_status)

                    with t2:
                        st.dataframe(df_d)
                        run_analysis_and_prompts(df_d, us_ticker, name_diplay, "US", "USD", "일봉", key_suffix="us_d", holding_status=holding_status)
                    with t3:
                        st.dataframe(df_w)
                        run_analysis_and_prompts(df_w, us_ticker, name_diplay, "US", "USD", "주봉", key_suffix="us_w", holding_status=holding_status)
                    with t4:
                        st.dataframe(df_m)
                        run_analysis_and_prompts(df_m, us_ticker, name_diplay, "US", "USD", "월봉", key_suffix="us_m", holding_status=holding_status)
                    with t5:
                        st.dataframe(df_y)
                        run_analysis_and_prompts(df_y, us_ticker, name_diplay, "US", "USD", "연봉", key_suffix="us_y", holding_status=holding_status)
                    ...
                    st.dataframe(df_final)
                    name_diplay = selected_us_name if us_name_to_ticker else us_ticker
                    run_analysis_and_prompts(df_final, us_ticker, name_diplay, "US", "USD", interval_us_sel, key_suffix="us_single", holding_status=holding_status)
            else:
